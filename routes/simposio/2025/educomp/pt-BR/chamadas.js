exports.topicos_interesse = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/topicos-interesse', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Tópicos de interesse',
    header: { titulo: 'Tópicos de interesse' },
  })
}

exports.artigos_completos = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/artigos-completos/artigos-completos', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Artigos completos',
    header: { titulo: 'Artigos completos' },
  })
}

exports.criterios_trilha_1 = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/artigos-completos/criterios-trilha-1', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Critérios - Trilha 1',
    header: { titulo: 'Critérios e diretivas para revisão' },
  })
}

exports.criterios_trilha_2 = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/artigos-completos/criterios-trilha-2', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Critérios - Trilha 2',
    header: { titulo: 'Critérios e diretivas para revisão' },
  })
}

exports.criterios_trilha_3 = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/artigos-completos/criterios-trilha-3', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Critérios - Trilha 3',
    header: { titulo: 'Critérios e diretivas para revisão' },
  })
}

exports.criterios_trilha_4 = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/artigos-completos/criterios-trilha-4', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Critérios - Trilha 4',
    header: { titulo: 'Critérios e diretivas para revisão' },
  })
}

exports.criterios_trilha_5 = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/artigos-completos/criterios-trilha-5', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Critérios - Trilha 5',
    header: { titulo: 'Critérios e diretivas para revisão' },
  })
}

exports.ctd = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/ctd', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Concurso de Teses e Dissertações',
    header: { titulo: 'Concurso de Teses e Dissertações' },
  })
}

exports.wtd = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/wtd', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Workshop de Teses e Dissertações',
    header: { titulo: 'Workshop de Teses e Dissertações' },
  })
}

exports.minicursos = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/minicursos', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Minicursos',
    header: { titulo: 'Minicursos' },
  })
}

exports.mesas_tematicas = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/mesas-tematicas', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Mesas temáticas',
    header: { titulo: 'Mesas temáticas' },
  })
}

exports.laboratorio_ideias = function (req, res) {
  res.render('simposio/2025/educomp/pt-BR/chamadas/laboratorio-ideias', {
    layout: 'simposio/2025/pt-BR/layout',
    chamadas: true,
    isEducomp: true,
    titulo: 'Laboratório de ideias',
    header: { titulo: 'Laboratório de ideias' },
  })
}
